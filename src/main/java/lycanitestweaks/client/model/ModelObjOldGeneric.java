...
public class ModelObjOldGeneric extends ModelCustomGeneric {

    // Copy of ModelObjOld without unnecessary casts from Entity -> EntityLiving
    ...
    //                    Constructors
    // ==================================================
    public ModelObjOldGeneric() {
        this(1.0F);
    }

    public ModelObjOldGeneric(float shadowSize) {
    ...
    //                    Init Model
    // ==================================================
    public ModelObjOldGeneric initModel(String name, ModInfo groupInfo, String path) {
        // Check If Enabled:
    ...
    public void render(Entity entity, float time, float distance, float loop, float lookY, float lookX, float scale, LayerCreatureBase layer, boolean animate) {
        ...
        if(scale < 0) {
            ...
            if(entity instanceof BaseCreatureEntity) {
                scale *= (float) ((BaseCreatureEntity)entity).getRenderScale();
            }
            else {
                scale *= this.getRenderScale(entity);
            }
        }
...
//        }
    ...
    public void animatePart(String partName, Entity entity, float time, float distance, float loop, float lookY, float lookX, float scale) {
        ...
        if(partName.toLowerCase().contains("head")) {
            if(!lockHeadX)
                rotX += (float) Math.toDegrees(lookX / (180F / (float)Math.PI));
            if(!lockHeadY)
                rotY += (float) Math.toDegrees(lookY / (180F / (float)Math.PI));
        }
    ...
    public void rotate(float rotation, float angleX, float angleY, float angleZ) {
        GlStateManager.rotate(rotation, angleX, angleY, angleZ);
    }
